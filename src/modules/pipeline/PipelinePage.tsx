...
interface Video { title: string; urgency: 'urgent' | 'warning' | 'normal'; urgencyText: string; channel: string; duration: string; style: string; assets: string; deadline: string; assignedBy: string; }
...
const INIT: PState = {
  steps:       { vid1: 5, vid2: 4, vid3: 2 },
  journal:     { vid1: { q1:'', q2:'', q3:'' }, vid2: { q1:'Showcase iPhone 16 Pro Max features', q2:'Tech user 25-40, đang dùng iPhone 13/14', q3:'Camera ProRes và Action Button' }, vid3: { q1:'', q2:'', q3:'' } },
  journalDone: { vid1: false, vid2: true, vid3: false },
  checks:      { vid1: { b5:[] }, vid2: { b2:[0,1,2], b3:[0,1,2,3], b4:[0,1] }, vid3: { b2:[] } },
  drive:       { vid1:'', vid2:'', vid3:'' },
};
const LS_KEY = 'nl2_pipeline_v2';

function loadState(): PState {
  try { const s = localStorage.getItem(LS_KEY); return s ? JSON.parse(s) : INIT; } catch { return INIT; }
}
...
function ModalStep1({ vid, state, setState, onClose, showToast }: ModalProps) {
  ...
  return (
    <>
      ...
      {[['1. Video này phục vụ mục đích gì?', q1, setQ1, 'Mục tiêu business/content của video này...'],
        ['2. Khán giả mục tiêu là ai?', q2, setQ2, 'Mô tả đối tượng xem video...'],
        ['3. Điều quan trọng nhất cần truyền đạt?', q3, setQ3, 'Key message / điểm chính cần nhấn mạnh...']
      ].map(([label, val, setter, ph]) => (
        <div key={label as string}>
          <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--t1)', margin: '14px 0 6px' }}>{label as string}</div>
          <textarea style={ta} placeholder={ph as string} value={val as string} onChange={e => (setter as (v: string) => void)(e.target.value)} />
        </div>
      ))}
...
function ModalChecklist({ vid, step, state, setState, onClose, showToast }: ModalProps & { step: number }) {
  ...
  const handleNext = () => {
    ...
    setState({ ...state, steps: { ...state.steps, [vid]: Math.max(state.steps[vid], nextStep) } });
    showToast(`✓ B${step} hoàn thành! Sang B${nextStep}`, 'success');
    onClose();
  };
...
function CardModal({ vid, state, setState, onClose, showToast }: { vid: string; state: PState; setState: (s: PState) => void; onClose: () => void; showToast: (msg: string, type?: Toast['type']) => void; }) {
  const v = VIDEOS[vid];
  const step = state.steps[vid];
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 500, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={onClose}>
      <div style={{ background: 'var(--bg2)', border: '1px solid var(--b2)', borderRadius: 14, padding: 24, width: 520, maxWidth: '90vw', maxHeight: '85vh', overflowY: 'auto', animation: 'popIn 0.25s cubic-bezier(0.34,1.56,0.64,1)' }} onClick={e => e.stopPropagation()}>
        {/* Header */}
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 2 }}>
          <div>
            <div style={{ fontFamily: 'Syne, sans-serif', fontWeight: 700, fontSize: 16, color: 'var(--t1)' }}>{v.title}</div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginTop: 5 }}>
              <span style={{ fontSize: 10, fontWeight: 700, color: 'var(--accent)', background: 'var(--accent2)', padding: '2px 8px', borderRadius: 4 }}>{STEP_ICONS[step]} B{step}: {STEP_NAMES[step]}</span>
              <span style={{ fontSize: 9, fontWeight: 700, padding: '2px 7px', borderRadius: 99, color: urgColor, background: v.urgency === 'urgent' ? 'var(--red2)' : v.urgency === 'warning' ? 'var(--amber2)' : 'var(--teal2)' }}>{v.urgencyText}</span>
            </div>
          </div>
        ...
        <div style={{ height: 1, background: 'var(--b1)', margin: '12px 0 14px' }} />
        {/* Body */}
        {step === 1 && <ModalStep1 vid={vid} state={state} setState={setState} onClose={onClose} showToast={showToast} />}
        {step >= 2 && step <= 5 && <ModalChecklist vid={vid} step={step} state={state} setState={setState} onClose={onClose} showToast={showToast} />}
        {step === 6 && <ModalStep6 vid={vid} state={state} setState={setState} onClose={onClose} showToast={showToast} />}
      </div>
    </div>
  );
}
...
export function PipelinePage() {
  ...
  const setState = (s: PState) => { setPState(s); localStorage.setItem(LS_KEY, JSON.stringify(s)); };
  const vidList = Object.keys(VIDEOS);
  const total = vidList.length;

  // Persist on change
  useEffect(() => { localStorage.setItem(LS_KEY, JSON.stringify(pState)); }, [pState]);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', margin: -18 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 18px 8px', flexShrink: 0, borderBottom: '1px solid var(--b1)' }}>
        ...
        <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--t1)' }}>{total} đang làm</span>
      ...
      <div style={{ position: 'fixed', bottom: 20, right: 20, zIndex: 9999, display: 'flex', flexDirection: 'column', gap: 8, pointerEvents: 'none' }}>
        {toasts.map(t => (
          <div key={t.id} style={{
            ...
            borderLeft: `3px solid ${t.type === 'success' ? 'var(--green)' : t.type === 'warn' ? 'var(--amber)' : 'var(--accent)'}`,
            borderRadius: 'var(--r)', padding: '10px 14px', fontSize: 12, color: 'var(--t1)',
            boxShadow: '0 4px 20px rgba(0,0,0,0.4)', animation: 'slideInFromRight4 0.3s ease',
          }}>
            {t.msg}
          </div>
        ))}
